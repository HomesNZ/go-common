...
func TestChildSpanFromGlobalTracer(t *testing.T) {
	...
	router := gin.New()
	router.Use(Middleware("foobar", []string{}))
...
func TestChildSpanFromCustomTracer(t *testing.T) {
	...
	router := gin.New()
	router.Use(Middleware("foobar", []string{}, WithTracerProvider(provider)))
	router.GET("/user/:id", func(c *gin.Context) {
		span := oteltrace.SpanFromContext(c.Request.Context())
		_, ok := span.(*oteltest.Span)
		assert.True(t, ok)
...
func TestPropagationWithCustomPropagators(t *testing.T) {
	...
	router := gin.New()
	router.Use(Middleware("foobar", []string{}, WithTracerProvider(provider), WithPropagators(b3)))
	router.GET("/user/:id", func(c *gin.Context) {
		span := oteltrace.SpanFromContext(c.Request.Context())
		mspan, ok := span.(*oteltest.Span)
		require.True(t, ok)
