...
func Middleware(service string, ignoredRoutes []string, opts ...Option) gin.HandlerFunc {
	cfg := config{}
	...
	return func(c *gin.Context) {
		...
		if isIgnoredRoute(spanName, ignoredRoutes) {
			c.Next()
			return
		}
...
func isIgnoredRoute(route string, ignoredRoutes []string) bool {
	for _, s := range ignoredRoutes {
		if strings.EqualFold(route, s) {
			return true
		}
	}
	return false
